...
function eliminarCaracter() {
    var inputString = document.getElementById("string2").value;
    ...
    if (posicion < 0 || posicion >= inputString.length) {
        ...
        var resultString = inputString.slice(0, posicion - 1) + inputString.slice(posicion);
...
function intercambiarCaracteres() {
    var inputString = document.getElementById("string3").value;
    if (inputString.length > 0) {
        var intercambiado = inputString[inputString.length - 1] + inputString.slice(1, inputString.length - 1) + inputString[0];
